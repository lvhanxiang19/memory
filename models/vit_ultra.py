...
from ..utils.ultra import UltraMemory
...
class UltraBlock(nn.Module):
    def __init__(
        ...
        self.ls1_1 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()
        self.ls1_2 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()
        self.ls1_3 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()

        self.ls2_1 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()
        self.ls2_2 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()
        self.ls2_3 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()
        ...
        self.drop_path1_3 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        ...
        self.drop_path2_3 = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.mlp_u = UltraMemory(
                input_dim=dim,
                output_dim=dim,
                key_dim=dim//2,
                value_dim=dim//2
        ...
        self.mlp_1 = mlp_layer(
            in_features=dim,
            hidden_features=int(dim * mlp_ratio),
            act_layer=act_layer,
            drop=proj_drop,
        )
        self.mlp_2 = mlp_layer(
            in_features=dim,
            hidden_features=int(dim * mlp_ratio),
            act_layer=act_layer,
            drop=proj_drop,
        )
        self.mlp_3 = mlp_layer(
            in_features=dim,
            hidden_features=int(dim * mlp_ratio),
            act_layer=act_layer,
            drop=proj_drop,
    ...
    def forward(self, x) -> torch.Tensor:
        x = x + self.drop_path1_1(self.ls1_1(self.attn1(self.norm1_1(x))))
        y=self.mlp_u(self.norm2_1(x))
        x = x + self.drop_path2_1(self.ls2_1(self.mlp_1(self.norm2_1(x))))

        x = x + self.drop_path1_2(self.ls1_2(self.attn2(self.norm1_2(x))))
        x = x + self.drop_path2_2(self.ls2_2(self.mlp_2(self.norm2_2(x))))

        x = x + self.drop_path1_3(self.ls1_3(self.attn3(self.norm1_3(x))))
        x = x + self.drop_path2_3(self.ls2_3(self.mlp_3(self.norm2_3(x))))
...
class VisionTransformer_ultra(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
            ...
            ultra_fn: Type[nn.Module] = UltraBlock,
        ...
        self.blocks = nn.Sequential(*[
            ...
            for i in range(3)])
        self.ultrablocks=nn.Sequential(*[
            ultra_fn(
                dim=embed_dim,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
...
@torch.no_grad()
def _load_weights(model: VisionTransformer_ultra, checkpoint_path: str, prefix: str = '') -> None:
...
def _create_vision_transformer_ultra(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer_ultra:
