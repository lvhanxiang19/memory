...
from ..utils.new_layer import newBlock
...
class VisionTransformer_new(nn.Module):
    ...
    def __init__(
            ...
            block_fn: Type[nn.Module] = newBlock,
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                mlp_layer=mlp_layer,
                # memory args
                is_alt = True if i in alt_layer else False,
                mem_dim=mem_dim,
                mem_k_dim=mem_k_dim,
                mem_k_num=mem_k_num,
                mem_head=mem_head,
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            x,loss = checkpoint_seq(self.blocks, (x,0))
        else:
            z=(x,0)
            x,loss = self.blocks(z)
        x = self.norm(x)
        return (x,loss)
...
@torch.no_grad()
def _load_weights(model: VisionTransformer_new, checkpoint_path: str, prefix: str = '') -> None:
...
def _create_vision_transformer_new(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer_new:
