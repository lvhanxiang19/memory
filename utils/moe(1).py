...
class TokenChoiceGating(nn.Module):
    ...
    def forward(self, key, query):
        ...
        *_, b, group_size, dim, dtype, top_n, num_experts, eps = *key.shape, key.dtype, self.top_n, self.num_experts, self.eps
        ...
        one_hot_gate_indices = F.one_hot(gate_indices, self.num_experts)
...
class ExpertChoiceGating(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.add_noise = add_noise
        self.noise_mult = noise_mult
        self.use_gather_scatter = use_gather_scatter
    ...
    def forward(self, key, query):
        ...
        *_, n, d, dtype, device = *key.shape, key.dtype, key.device
        ...
        if self.use_gather_scatter:
            return topk_indices, topk_scores, balance_loss, router_z_loss
...
class MoE(nn.Module):
    def __init__(        
        ...
        **kwargs: Any,
        ...
        self.router = self.router_func(
            ...
            #
            use_gather_scatter = self.use_gather_scatter,
        )
    ...
    def forward(self, x):
        ...
        router_output = self.router(key, query)

        if self.use_gather_scatter and self.router_func == ExpertChoiceGating:
            b, n, d = x.shape
            topk_indices, topk_scores, balance_loss, router_z_loss = router_output  # b e c
            gather_indices = topk_indices.unsqueeze(-1).expand(-1, -1, -1, d) 
            slots = torch.gather(x.unsqueeze(1).expand(-1, self.num_experts, -1, -1), 2, gather_indices)             
        else:
            dispatch_tensor, combine_tensor, balance_loss, router_z_loss = router_output
            # dispatch
        ...
        if self.use_gather_scatter and self.router_func == ExpertChoiceGating:
            ...
            # 加权专家输出
            weighted_outputs = outputs * topk_scores.unsqueeze(-1)
            weighted_outputs_flat = weighted_outputs.view(b, -1, d)
            indices_flat = topk_indices.view(b, -1)
            indices_flat_expanded = indices_flat.unsqueeze(-1).expand(-1, -1, d)
            output.scatter_add_(1, indices_flat_expanded, weighted_outputs_flat)
        else:
            # 👇 原始的显式张量模式
            output = torch.einsum('b e s d, b n e s -> b n d', outputs, combine_tensor)
